...
class StoreKpiFieldRelationRequest extends FormRequest
{
    ...
    {
        return [
            'field_a_id' => 'required|exists:kpi_fields,id',
            'field_b_id' => 'required|exists:kpi_fields,id|different:field_a_id',
            'operation' => 'required|in:divide,multiply,add,subtract,percentage',
    ...
    {
        return [
            'field_a_id.required' => 'El primer campo es obligatorio.',
            'field_a_id.exists' => 'El primer campo debe existir.',
            'field_b_id.required' => 'El segundo campo es obligatorio.',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Validar que ambos campos pertenecen al mismo KPI
            ...
            // Validar que no existe una relación duplicada
            ...
            // Validar que no se exceda el límite de relaciones por KPI
            if ($kpiId) {
                $relationCount = \App\Models\Dashboard\KpiFieldRelation::where('kpi_id', $kpiId)->count();
                if ($relationCount >= 1) { // Máximo 1 relación por KPI
                    $validator->errors()->add('kpi_id', 'Este KPI ya tiene el máximo de relaciones permitidas (1).');
                }
            }
        });
    }
